...
function objectWithArrayValues(array1, array2) {
    ...
    for (const value of array1) {
        result[value] = array2.map((callback) => callback(value));
    }
...
```js
function schedule(functions, seconds) {
    if (functions.length !== seconds.length) {
        alert('Invalid input: The two arrays must have the same length.');
        return;
    }

    for (let i = 0; i < functions.length; i++) {
        setTimeout(functions[i], seconds[i] * 1000);
    }
}
function sayHi() {
  console.log('Hi');
